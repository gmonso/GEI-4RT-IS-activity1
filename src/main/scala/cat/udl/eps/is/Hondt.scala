...
/**
 * Aquest objecte conté una funció que rep els vots rebuts per cada partit i el nombre d'escons a repartir
 *
 * NOTA: M'ha resultat fàcil d'implementar, ja que has definit l'estratègia a seguir, i si només ens fixàvem amb els tipus que have d'anar convertint, era fàcil de seguir.
 */
object Hondt {
   ...
   *
   * @param votes vots rebuts per cada partit
   * @param seats nombre d'escons a repartir
   * @return nombre d'escons per partit resultant
   */
  def hondt(votes: Map[String, Int], seats: Int): Map[String, Int] = {
     ...
     *
     * @param nvotes nombre de vots
     * @return llista de quocients
     */
    def quotients(nvotes: Int): List[Double] = { /** recorrem els seients del 1 al seats i creem una llista de Double de la divisió del element del loop amb el num de vots passat per paràmetre. */
      (1 to seats).foldLeft(List())((acc: List[Double], elem) => {
